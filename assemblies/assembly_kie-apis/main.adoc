...
+
Although these user roles are not all required for every KIE API, consider acquiring all of them to ensure that you can access any KIE API without disruption. For more information about user roles, see the {URL_PLANNING_INSTALL}[_{PLANNING_INSTALL}_].
...
include::{shared-dir}/Commands/runtime-commands-marshalling-ref.adoc[leveloffset=+2]
...
include::{shared-dir}/KieServer/kie-server-commands-marshalling-ref.adoc[leveloffset=+2]
