//@Jason: During the split, the first two items items below need to be modified or added: The anchor ID needs to change from 'chap-...' to 'assembly-...', and the variable `:KOGITO-ENT:` needs to be added (to activate all the enterprise conditions). Everything else is as it appears in the chap file, except that the module sections are converted to includes.
