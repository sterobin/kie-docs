...
Endpoints::
{CONTROLLER} REST API endpoints, such as `/management/servers/{serverId}` for a specified {KIE_SERVER} template, are the URIs that you append to the {CONTROLLER} base URL to access the corresponding server resource or type of server resource in {PRODUCT}.
+
--
