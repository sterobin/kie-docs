...
*<@Maciej: Can you re-validate this section now, especially the new Java request examples? I'm not confident in my explanations nor in my Java request examples.>*
...
Each command in this section includes a JSON request body example for the {KIE_SERVER} REST API and an embedded method from the `KieServicesClient` Java client for the {KIE_SERVER} Java client API.
...
|`container`
|Map containing the `container-id`, `release-id` (group ID, artifact ID, version) data, `status`, and any other components of the new KIE container
|Required
|===
...
*<@Maciej: Can't figure out what this Java request example should be. Needs to have a status and I don't know where.>*

.Example Java request
[source,java]
----
ServiceResponse<KieScannerResource> response = kieServicesClient.updateScanner("command-script-container", resource);
----
