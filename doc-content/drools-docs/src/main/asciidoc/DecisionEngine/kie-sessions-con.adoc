...
In {PRODUCT}, a KIE session stores and executes runtime data. The KIE session is automatically created from a KIE base or directly from a KIE container if you have defined the KIE session in the KIE module descriptor file (`kmodule.xml`) for your project.
