...
For this example, if the first rule in the `"report"` activation group is executed, the second rule in the group and all other executable rules on the agenda must be removed from the agenda. Therefore, the `"report"` group must receive the focus to be executed first, and only, among other rules on the agenda if the rule conditions are met:
