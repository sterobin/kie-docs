...
A stateless KIE session is a session without inference. In a stateless KIE session, data from a previous invocation of the KIE session (the previous session state) is discarded between session invocations. In a stateful KIE session, that data is retained. A stateless KIE session can be called like a function in that you can use it to pass data and then receive the result back.
...
[NOTE]
====
Use of the dollar sign (`$`) is optional and helps to differentiate between variable names and field names.
====
...
In this case, the specified applicant is under the age of eighteen, so the application is declined.
