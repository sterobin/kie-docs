...
The same bit-mask technique is used to track modified nodes, segments, and rules. This tracking ability enables an already linked rule to be scheduled for evaluation if it has been modified since it was last evaluated. As a result, no rules can ever evaluate partial matches.
