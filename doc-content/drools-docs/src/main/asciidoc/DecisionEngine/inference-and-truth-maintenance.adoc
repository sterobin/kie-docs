...
The basic function of the {DECISION_ENGINE} is to match production facts and data to rules and determine whether and how rules are executed. To ensure that relevant data is applied to the appropriate rules, the {DECISION_ENGINE} makes inferences based on existing knowledge and performs the actions based on the inferred information.
...
In many cases, new data in a rule system is the result of other rule executions, and this new data can affect the execution of other rules. If the {DECISION_ENGINE} asserts data as a result of executing a rule, the {DECISION_ENGINE} uses truth maintenance to justify the assertion and enforce truthfulness when applying inferred information to the rule. Truth maintenance also helps to identify inconsistencies and to handle contradictions. For example, if two rules are executed and result in a contradictory action, the {DECISION_ENGINE} chooses the action based on assumptions from previously calculated conclusions.

The {DECISION_ENGINE} inserts facts using either stated or logical insertions:

* *Stated insertions:* Defined with `insert()`. After stated insertions, facts are generally retracted explicitly.
* *Logical insertions:* Defined with `insertLogical()`. After logical insertions, the facts that were inserted are automatically retracted when the conditions that inserted the facts are no longer true. The facts are retracted when there is no single condition that supports the logical insertion.

The following flowcharts illustrate the life cycle of stated and logical insertions:

.Stated insertion
image::HybridReasoning/7165.png[]

.Logical insertion
image::HybridReasoning/7166.png[]

IMPORTANT: For logical insertions, your fact objects must override the `equals` and `hashCode` methods from `java.lang.Object` according to the Java standard. Two objects are equal if their `equals` methods return `true` for each other and if their `hashCode` methods return the same values. For more information, see the Java API documentation for your Java version.
...
When the condition in the rule is false, the fact is automatically retracted. This behavior is helpful in this example because the two rules are mutually exclusive. In this example, if the person is younger than 16 years old, the rule logically inserts an `IsChild` fact. Once the person is 16 years old or older, the `IsChild` fact is automatically retracted and the `IsAdult` fact inserted.
...
When a person turns 16 years old, the `IsChild` fact and the person's `ChildBusPass` fact is retracted. To these set of conditions, you can relate another rule that states that a person must return the child pass after turning 16 years old. When the {DECISION_ENGINE} automatically retracts the `ChildBusPass` object, this following is executed and sends a request to the person:
