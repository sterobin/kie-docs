...
For example, the following two DRL rules sound a fire alarm based on an average temperate. However, the first rule uses a sliding time window to calculate the average over the last ten minutes while the second rule uses a sliding length window to calculate the average over the last one hundred temperature readings.
...
The {DECISION_ENGINE} discards any `SensorReading` events that are more than ten minutes old or that are not part of the last one hundred readings, and continues recalculating the average as minutes or readings "slide" forward.
