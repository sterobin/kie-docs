...
+
--
This operator defines an amount of time after which the current event can follow the correlated event, or a delimiting time range during which the current event can follow the correlated event.
...
before::
This operator defines an amount of time after which the current event can precede the correlated event, or a delimiting time range during which the current event can precede the correlated event.
+
--
For example, the following pattern matches if `$eventB` starts between 3 minutes and 30 seconds and 4 minutes after `$eventA` finishes. If `$eventB` starts earlier than 3 minutes and 30 seconds after `$eventA` finishes, or later than 4 minutes after `$eventA` finishes, then the pattern is not matched.
