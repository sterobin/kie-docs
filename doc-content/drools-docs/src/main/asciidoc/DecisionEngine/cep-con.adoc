...
* On an algorithmic trading application: Perform an action if the security price increases by X percent above the day opening price. The price increases are denoted by events on a stock trade application.
* On a monitoring application: Perform an action if the temperature in the server room increases X degrees in Y minutes. The sensor readings are denoted by events.
...
* Scenarios usually process large numbers of events, but only a small percentage of the events are of interest
...
* Individual events are not prioritized. The system prioritizes patterns of related events and the relationships between them.
...
* Events with their proper semantics
...
* Required volumes of events for complex event processing use cases
* Reactive rules
