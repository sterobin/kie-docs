...
:PRODUCT_INIT: kogito
:DEVELOPMENT_MODE: development mode or in native mode
:DECISION_SERVICES_LINK: xref:con-drl-rule-units_drl-rules[]
:PROCESS_SERVICES_LINK: xref:chap-kogito-developing-process-services[]
:RUNTIME_EVENTS_LINK: xref:con-kogito-runtime-events_kogito-configuring[]
:DATA_INDEX_SERVICE_LINK: xref:con-data-index-service_kogito-configuring[]
:DECISION_TABLES_LINK: xref:con-decision-tables_decision-tables[]
:CREATING_RUNNING_LINK: xref:proc-kogito-running-app_kogito-creating-running[]
:DECISION_RULES_LINK: xref:con-drl_drl-rules[]
:RULE_UNITS_DRL_LINK: xref:con-drl-rule-units_drl-rules[]
:DECISION_DMN_MODELS_LINK: xref:con-dmn_dmn-models[]
:EVENT_LISTENERS_REGISTERING_LINK: xref:proc-event-listeners-registering_kogito-configuring[]
:DEPLOYING_ON_OPENSHIFT_CLI_LINK: xref:proc-kogito-deploying-on-ocp-kogito-cli_kogito-deploying-on-openshift[]
:JOBS_SERVICE_LINK: xref:con-jobs-service_kogito-configuring[]
:MANAGEMENT_CONSOLE_LINK: xref:con-management-console_kogito-developing-process-services[]
:DEPLOYING_ON_OPENSHIFT_LINK: xref:con-kogito-on-ocp_kogito-deploying-on-openshift[]
:PERSISTENCE_LINK: xref:con-persistence_kogito-configuring[]
:ENABLING_PERSISTENCE_LINK: xref:proc-persistence-enabling_kogito-configuring[]
:ENABLING_MESSAGING_LINK: xref:proc-messaging-enabling_kogito-configuring[]
:TASK_LIFE_CYCLE_LINK: xref:con-task-life-cycle_kogito-developing-process-services[]
endif::[]
...
[IMPORTANT]
====
{PRODUCT} is currently available for Development Preview.
Development Preview releases contain features and APIs that might not be fully tested and that might change in the final GA version.
Users are discouraged from using Development Preview software in production or for business-critical workloads.
Because this is not production-ready software, users are not able to open support tickets.
To learn more about {PRODUCT}, please contact your Red Hat representative or send an email to Kogito-earlyaccess@redhat.com.
Red Hat will address reported issues at its own discretion.
====
...
NOTE: For the predefined {PRODUCT} example applications, the Swagger UI for interacting with service endpoints is available only on Quarkus examples that you run in {DEVELOPMENT_MODE}.
...
{PRODUCT} provides a https://kiegroup.github.io/kogito-online/#/download[Business Modeler Hub] desktop application with all {PRODUCT} extensions or applications for modeling Business Process Model and Notation (BPMN) process models and Decision Model and Notation (DMN) decision models in {PRODUCT} services.
{PRODUCT} currently provides extensions for VSCode (recommended) and GitHub modelers, a desktop application for offline modeling, and an online modeler.
...
Visual Studio Code (VSCode) is the preferred integrated development environment (IDE) for developing {PRODUCT} services.
{PRODUCT} provides a https://github.com/kiegroup/kogito-tooling/releases[VSCode extension] that enables you to design Business Process Model and Notation (BPMN) 2.0 business processes and Decision Model and Notation (DMN) decision models directly in VSCode.
...
Use this method to open VSCode each time you develop {PRODUCT} services.
This set of parameters enables the Microsoft https://code.visualstudio.com/api/advanced-topics/using-proposed-api[Proposed API] for an optimal user experience with VSCode extensions.
--
...
NOTE: For new BPMN or DMN models, you can also enter `bpmn.new` or `dmn.new` in a web browser to design your BPMN or DMN model in the {PRODUCT} online modeler.
When you finish creating your model, you can click *Download* in the online modeler page to import your BPMN or DMN file into your {PRODUCT} project in VSCode.
...
[id='proc-kogito-designing-app-rule-units_{context}', xreflabel='Using DRL rule units as an alternative decision service']
...
For more information about rule units, see {DECISION_SERVICES_LINK}.
...
// TODO: I don't know if anything needs to happen here - Jason
== Additional resources
