...
The example service uses the following `MonitoringService.drl` file in the same `src/main/resources/org/acme` folder of the {PRODUCT} project to declare the rule unit with the associated data sources and define the rules:
...
The rule unit implements the required `RuleUnitData` interface and identifies the previously declared data sources. The sample rule raises an alert when the sensor temperature reaches 80 degrees or higher.
...
== Rule unit declaration using Java
...
== Rule units with BPMN processes
