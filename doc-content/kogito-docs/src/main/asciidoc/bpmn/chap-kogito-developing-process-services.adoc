...
+
--
* *Process Instances*: Use this page to view and filter process and subprocess instances by status or business key. You can select a specific process instance name to view process details or abort, skip, or retry the process. You can also select the check box for multiple instances and use the bulk operation options at the top of the screen.
+
...
+
For jobs in a *Scheduled* or an *Error* state, you can also select the *Reschedule* option to reschedule the jobs. You can modify the expiration time for one-time jobs, or the expiration time, repeat interval, and repeat limit for periodic jobs.
+
...
+
The *Node Trigger* panel displays a list of nodes that you can select from the drop-down menu and trigger.
+
