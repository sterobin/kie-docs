[id='proc_dmn-logic-defining_{context}']
= Defining DMN decision logic in boxed expressions in the {PRODUCT} DMN designer
...
You can use the {PRODUCT} DMN designer in VSCode to define decision logic for your DRD components using built-in boxed expressions.
...
For more information and examples of each boxed expression type, see xref:con_dmn-boxed-expressions_dmn-models[].
