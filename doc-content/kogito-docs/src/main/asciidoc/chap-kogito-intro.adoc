[id='chap-kogito-intro']

////
//@Jason: Content came from these includes, note level offset. Also note no overall chapter title in this case because these includes were in the straight index.adoc without being in a chap file until now. So the first section functions as the overall chapter title basically. As a result it makes these chap basically have double anchor IDs: One for the chap file followed by nothing followed by the anchor ID of the first section. Can add empty text if needed.
